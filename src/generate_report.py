...
def load_period_trends(days: int) -> list[dict]:
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        for sport_type in types:
            rows = conn.execute("SELECT * FROM sport_daily_history WHERE sport_type = ? AND date >= ? ORDER BY date ASC", (sport_type, cutoff)).fetchall()
            if len(rows) < 2:
            ...
            trends.append({
                "sport_type": sport_type, "days": days, "status": coverage_status(days, covered_days), "covered_days": covered_days,
                "races": races_delta, "wins": wins_delta, "top5": top5_delta, "poles": poles_delta,
...
def load_dr_trend(days: int) -> dict:
    ...
    with sqlite3.connect(DB_PATH) as conn:
        conn.row_factory = sqlite3.Row
        rows = conn.execute("SELECT captured_at, dr_points, dr_percentage, driver_rating, dr_label FROM rating_history WHERE captured_at >= ? ORDER BY captured_at ASC", (cutoff,)).fetchall()
    if len(rows) < 2:
        return {"status": "insufficient_data"}
    first, last = rows[0], rows[-1]
    covered_days = date_span_days(first["captured_at"], last["captured_at"])
    return {"status": coverage_status(days, covered_days), "covered_days": covered_days,
            "dr_points_change": delta(last["dr_points"], first["dr_points"]),
            "dr_percentage_change": delta(last["dr_percentage"], first["dr_percentage"]),
            "from_label": first["dr_label"], "to_label": last["dr_label"]}


def main():
    rating = json.loads(RATING_PATH.read_text(encoding="utf-8"))
    career = json.loads(CAREER_PATH.read_text(encoding="utf-8"))
    lines = ["# GT7 Sport Career Report", "", f"PSN: **{rating['psn_id']}**  ", f"Updated: {career['captured_at']}  ",
             f"DR: **{rating['dr_label']}** — {rating['dr_points']} points — {rating['dr_percentage']}% toward next DR  ",
             f"SR: **{rating['sportsmanship_rating']}**", "", "## Sport career", "",
    ...
    for row in career.get("sport_types", []):
        lines.append(f"| {sport_label(row['sport_type'])} | {row['races']} | {row['wins']} | {row['top5']} | {row['poles']} | {num(row['average_grid'])} | {num(row['average_finish'])} | {num(row['positions_gained_avg'])} | {pct(row['win_rate'])} | {pct(row['top5_rate'])} | {pct(row['pole_rate'])} |")

    sm = career.get("sports_mode", {})
    qp = career.get("qualifying_performance", {})
    lines += ["", "## Sports Mode counters", "", f"Races: **{sm.get('race_count')}** · Wins: **{sm.get('win_count')}** · Poles: **{sm.get('pole_position_count')}** · Fastest laps: **{sm.get('fastest_lap_count')}** · Clean races: **{sm.get('clean_race_count')}**",
              "", "## Qualifying performance", "", f"Best rank: **{qp.get('best_rank')}** · Median rank: **{qp.get('median_rank')}** · Average rank: **{qp.get('average_rank')}** · Worst rank: **{qp.get('worst_rank')}** · Rank standard deviation: **{qp.get('rank_stddev')}**", "", "## Trends", ""]
    ...
    for days in (7, 30, 90):
        ...
        if dr.get("status") == "insufficient_data":
            ...
            note = "" if dr.get("status") == "ok" else f" · partial coverage: {dr.get('covered_days', 0)} days"
            lines.append(f"DR: {dr['from_label']} → {dr['to_label']} · points {signed(dr['dr_points_change'], 0)} · progress {signed(dr['dr_percentage_change'], 1)} pp{note}")
        for tr in load_period_trends(days):
            ...
            if tr.get("status") == "insufficient_data":
                ...
                continue
            note = "" if tr.get("status") == "ok" else f" · partial coverage: {tr.get('covered_days', 0)} days"
            lines.append(f"- {label}: {tr['races']} races · {tr['wins']} wins · {tr['top5']} Top 5 · {tr['poles']} poles · win rate {pct(tr['period_win_rate'])} · Top-5 rate {pct(tr['period_top5_rate'])} · avg grid change {signed(tr['avg_grid_change'])} · avg finish change {signed(tr['avg_finish_change'])}{note}")
        lines.append("")
    ...
    lines += ["- Positive grid/finish change means the numerical average position increased; because lower position numbers are better, negative changes indicate improvement.",
              "- Trend windows explicitly flag partial coverage until the database contains enough history for the requested 7/30/90-day period.",
              "- Raw GTSH sport_type values remain stored unchanged in SQLite; human-readable labels are applied only in reports.",
              "- GTSH Sport totals and monthly Sports Mode counters are kept separate because they currently use different definitions/populations."]
