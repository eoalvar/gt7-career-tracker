...
def load_cache():
    ...
    try:
        ...
        obj.setdefault("profiles", {}); obj.setdefault("unavailable", {}); obj.setdefault("filtered", {})
        return obj
    except Exception as exc:
...
def save_cache(cache):
    ...
    payload = {"updated_at": datetime.now(timezone.utc).isoformat(), "target_valid_profiles": TARGET_VALID_REFERENCE, **cache}
    CACHE_PATH.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")


def norm(psn): return psn.casefold()

def record_status(bucket, psn, reason=None):
    bucket[norm(psn)] = {"psn_id": psn, "reason": reason, "last_attempt_at": datetime.now(timezone.utc).isoformat()}


def fetch_profile_with_rating(session: requests.Session, psn: str):
    """Fetch the same career metrics as base plus current DR/DR-points from GTSH."""
    url = base.PROFILE_BASE + psn
    page = session.get(url, timeout=30); page.raise_for_status(); soup = BeautifulSoup(page.text, "html.parser")
    body = soup.find("body"); key = body.get("header") if body else None
    ...
    response.raise_for_status(); wrapper = response.json(); enc = wrapper.get("data") if isinstance(wrapper, dict) else None
    if not isinstance(enc, str): return None, ["missing_encrypted_payload"]
    payload = json.loads(base.xor_decrypt(base64.b64decode(enc), key))
    ...
    reasons = base.validate_metrics(races,wins,top5,avg_grid,avg_finish)
    if reasons: return None, reasons
    user = payload.get("monthly_stats", {}).get("result", {}).get("user", {})
    dr_code = user.get("driver_rating") if isinstance(user, dict) else None
    dr_points = user.get("dr_points") if isinstance(user, dict) else None
    if not isinstance(dr_code, (int,float)): return None, ["missing_driver_rating"]
    row={"psn_id":psn,"races":int(races),"wins":int(wins),"top5":int(top5),"average_grid":float(avg_grid),"average_finish":float(avg_finish),"driver_rating":int(dr_code),"dr_label":user.get("dr_level"),"dr_points":float(dr_points) if isinstance(dr_points,(int,float)) else None,"dr_percentage":user.get("dr_percentage")}
    row["positions_gained_avg"]=row["average_grid"]-row["average_finish"]; row["win_rate"]=wins/races; row["top5_rate"]=top5/races
    return row, []
...
def profile_with_retry(session, psn):
    ...
    for attempt in range(1,4):
        try: return fetch_profile_with_rating(session, psn)
            ...
            last_exc=exc; time.sleep(2 ** attempt)
...
def select_dr_peers(sample, rating):
    user_dr = int(rating["driver_rating"]); user_pts = rating.get("dr_points")
    same = [r for r in sample if r.get("driver_rating") == user_dr]
    if isinstance(user_pts,(int,float)):
        rated = [r for r in same if isinstance(r.get("dr_points"),(int,float))]
        if len(rated) >= DR_MIN_PEERS:
            rated.sort(key=lambda r: abs(float(r["dr_points"])-float(user_pts)))
            peers = rated[:DR_MAX_PEERS]
            return peers,{"method":"same_dr_nearest_points","driver_rating":user_dr,"peer_count":len(peers),"dr_points_min":min(r["dr_points"] for r in peers),"dr_points_max":max(r["dr_points"] for r in peers)}
    peers = list(same)
    radius = 0
    while len(peers) < DR_MIN_PEERS and radius < 2:
        radius += 1
        peers = [r for r in sample if isinstance(r.get("driver_rating"),(int,float)) and abs(int(r["driver_rating"])-user_dr) <= radius]
    if isinstance(user_pts,(int,float)):
        peers.sort(key=lambda r: (abs(int(r["driver_rating"])-user_dr), abs(float(r.get("dr_points",user_pts))-float(user_pts))))
    peers = peers[:DR_MAX_PEERS]
    return peers,{"method":"dr_band_fallback","driver_rating":user_dr,"dr_radius":radius,"peer_count":len(peers)}


def adjusted_percentiles(user, sample, rating):
    peers, meta = select_dr_peers(sample, rating)
    if not peers: return None, meta
    scores = base.metric_percentiles(user, peers)
    # Recondition racecraft on both DR peer set and similar grid position.
    rc, rc_meta = base.conditioned_racecraft(user, peers)
    scores["racecraft_unconditioned"] = scores.get("positions_gained_avg")
    scores["racecraft"] = rc
    scores["racecraft_conditioning"] = rc_meta
    scores["overall"] = .30*scores["qualifying"] + .35*scores["race_performance"] + .20*scores["racecraft"] + .15*scores["results"]
    meta["racecraft_peer_count"] = rc_meta.get("peer_count")
    meta["racecraft_grid_window"] = rc_meta.get("grid_window")
    return scores, meta


def main():
    session=requests.Session(); session.headers.update(base.HEADERS); cache=load_cache(); rating=json.loads(RATING_PATH.read_text(encoding="utf-8")); existing=len(cache["profiles"])
    print(f"Cumulative reference before run: {existing}/{TARGET_VALID_REFERENCE} valid profiles")

    # Existing pre-DR cache rows are deliberately refreshed before new sampling.
    ...
    print(f"Cumulative DR-aware reference: {cumulative}/{TARGET_VALID_REFERENCE} ({cumulative/TARGET_VALID_REFERENCE:.1%})"); print("DR adjustment:",json.dumps(adjust_meta,indent=2)); print(json.dumps(adjusted,indent=2))
